...
class ExcelAgent(BaseAgent):
    ...
    def get_out(self):
        ...
        for cellname in self.chars:
            try:
                out.append(float(self.model.eval(cellname)))
            except Exception as exc:
                raise Exception(f"Something led to cell {cellname} not being set! ({exc})")
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        self.renderer.draw_string_2d(0, 20, 2, 2, self.model.eval("L27"), self.renderer.white())
        ...
        if self.i == 30:
            self.i = 0
            if DEBUG:
                with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "state.txt"), "w") as f:
